...
const char* g_lpszDocumentFieldList = "DOCUMENT_GUID, DOCUMENT_TITLE, DOCUMENT_LOCATION, DOCUMENT_URL, DOCUMENT_TAG_GUIDS, DOCUMENT_TYPE, DOCUMENT_FILE_TYPE, DT_CREATED, DT_MODIFIED, DOCUMENT_DATA_MD5, ATTACHMENT_COUNT, SERVER_CHANGED, LOCAL_CHANGED,DOCUMENT_ATTACHEMENT_COUNT";
...
bool CIndex::UpdateDocument(const WIZDOCUMENTDATA& data)
{
	...
	if (DocumentFromGUID(data.strGUID.c_str(), dataExists))
	{
        ...
        std::string strProtected = (data.nProtected == 1 || dataExists.nProtected)?"1":"0";
		//
		...
		+ ", SERVER_CHANGED=" + strServerChanged
		+ ", LOCAL_CHANGED=" + strLocalChanged
        + ", DOCUMENT_ATTACHEMENT_COUNT=" + strProtected;
	...
	else
	{
		...
		sql = std::string("insert into WIZ_DOCUMENT (DOCUMENT_GUID, DOCUMENT_TITLE, DOCUMENT_LOCATION, DOCUMENT_URL, DOCUMENT_TAG_GUIDS, DOCUMENT_TYPE, DOCUMENT_FILE_TYPE, DT_CREATED, DT_MODIFIED, DOCUMENT_DATA_MD5, ATTACHMENT_COUNT, SERVER_CHANGED, LOCAL_CHANGED,DOCUMENT_ATTACHEMENT_COUNT) values (")
...
bool CIndex::SQLToDocuments(const char* lpszSQL, CWizDocumentDataArray& arrayDocument)
{
	...
	try {
		...
		while (!query.eof())
		{
            ...
            data.nProtected = query.getIntField(13);
			//
