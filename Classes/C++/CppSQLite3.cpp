...
void CppSQLite3Binary::setEncoded(const unsigned char* pBuf)
{
	...
	setEncoded(pBuf, (int)strlen((const char*)pBuf));
}


void CppSQLite3Binary::setEncoded(const unsigned char* pBuf, int nSize)
{
	clear();
...
#ifdef _DEBUG
		int n2 = GetTickCount();
		TOLOG(CString(_T("sqlite3_encode_binary=")) + WizIntToStr(n2 - n1));
#endif
...
__int64 CppSQLite3Query::getInt64Field(const char* szField, int nNullValue/*=0*/)
{
	int nField = fieldIndex(szField);
...
bool CppSQLite3Query::getBoolField(const char* szField, bool bNullValue/*=false*/)
{
	int nField = fieldIndex(szField);
...
//{
//	CString str = getStringField16(nField);
//	if (str.IsEmpty())
//	{
//		if (0 == tNullValue)
...
//	else
//	{
//		return CppSQLiteHelper::StringToDateTime(str);
//	}
//}
//COleDateTime CppSQLite3Query::getTimeField(const char* szField, time_t tNullValue /*= 0*/)
//{
//	int nField = fieldIndex(szField);
...
//	else
//	{
//		return CppSQLiteHelper::StringToColor(str);
//	}
//}
//COLORREF CppSQLite3Query::getColorField(const char* szField, COLORREF crNullValue /*= 0*/)
//{
//	int nField = fieldIndex(szField);
...
void CppSQLite3Query::finalize()
{
	if (mpVM && mbOwnVM)
	{
		...
		{
			const char* szError = sqlite3_errmsg(mpDB);
			throw CppSQLite3Exception(nRet, (char*)szError);
		}
	}
}
...
int CppSQLite3DB::updateBlob(const char* szTableName, const char* szFieldName, const unsigned char* data, int dataLength, const char* szWhere)
{
    
    std::string sql = std::string("update ") + szTableName + " set " + szFieldName + "=? where " + szWhere;

    CppSQLite3Statement statement = compileStatement(sql.c_str());
...
int CppSQLite3DB::insertBlob(const char *szSQL, const unsigned char *data, int dataLength, int dataPosition)
{
    CppSQLite3Statement statement = compileStatement(szSQL);
    
    
    statement.bind(dataPosition, data, dataLength);
    return statement.execDML();
}
...
bool CppSQLite3DB::hasRecord(const char* lpszSQL)
{
	try {
		checkDB();
		//
		CppSQLite3Query query = execQuery(lpszSQL);
		return !query.eof();
	}
	...
	{
		TOLOG("Unknown error while hasRecord");
		return false;
	}
}
...
int sqlite3_encode_binary(const unsigned char *in, int n, unsigned char *out)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		int char1 = b / 16;
		int char2 = b % 16;
		*pRet = (char1 < 10) ? char1 + '0' : char1 - 10 + 'A';
		pRet++;
		*pRet = (char2 < 10) ? char2 + '0' : char2 - 10 + 'A';
		pRet++;
	}
	*pRet = 0;
	return n * 2;
